...
class Freqlog:
    ...
    def _on_key(self, key: libvinput.KeyboardEvent) -> None:
    ...
    def _any_modifier_enabled(self, modifiers) -> bool:
        return modifiers.left_control or modifiers.right_control or \
                modifiers.left_alt or modifiers.right_alt or modifiers.left_meta or \
                modifiers.right_meta or modifiers.left_super or modifiers.right_super or \
                modifiers.left_hyper or modifiers.right_hyper
    ...
    def _process_queue(self):
        ...
        while self.is_logging:
            try:
                action: ActionType
                modifiers: libvinput.KeyboardModifiers
                ...
                if ((isinstance(key, str) and key in " \t\n\r") or
                        (isinstance(key, str) and (not key or key not in self.allowed_chars))):
                ...
                if isinstance(key, str) and not self._any_modifier_enabled(modifiers) and key:
    ...
    def __init__(self, backend_path: str, password_callback: callable, loggable: bool = True,
        ...
        self.listener = None
        if loggable:
            self.listener = libvinput.listener_create(True)
    ...
    def start_logging(self, new_word_threshold: float | None = None, chord_char_threshold: int | None = None,
                      allowed_chars: set | str | None = None, allowed_first_chars: set | str | None = None) -> None:
        ...
        self.listener_thread = Thread(target=lambda: libvinput.listener_start(
            self.listener, lambda x: self._on_key(x),
    ...
    def stop_logging(self) -> None:  # FIXME: find out why this runs twice on one Ctrl-C (does it still?)
        ...
        if self.listener:
            libvinput.listener_destroy(self.listener)
            self.listener = None
        self.is_logging = False
